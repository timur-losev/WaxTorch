...
- **Frame kinds involved**: Core frame model + surrogate frames (identified by `WAXSURR1` content prefix; no new binary-format frame kind field, detection via payload magic header).
...
- **Token budget impact**: BPE `TokenCounter` (cl100k_base) ported with dual-mode (exact BPE via greedy merge / byte-level `~4 bytes/token` estimation); `ChunkText()` utility and `ChunkContentTokenAware()` orchestrator path provide token-aware chunking when vocab is loaded (whitespace fallback when no vocab). `ExtractiveSurrogateGenerator` accepts optional `TokenCounter*` for accurate tier budget enforcement (`full_max_tokens/gist_max_tokens/micro_max_tokens`). `OptimizeSurrogates()` passes tier config through to generator.
...
5. `ChunkContentTokenAware()` is wired but not yet called from `Remember()`; activation requires passing a `TokenCounter*` through `OrchestratorConfig` or constructing one internally (vocab path resolution needed). Current default remains whitespace-word-based chunking.
6. Remaining unported Swift RAG modules: `Ingest` pipeline, `PhotoRAG`, `VideoRAG`, `DateParser`, `SurrogateTierSelector` (runtime tier selection for recall context), and `ScheduledLiveSetMaintenance` (periodic compaction/rewrite scheduling).
