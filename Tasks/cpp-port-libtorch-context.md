...
6. Remaining unported Swift RAG modules: `Ingest` pipeline, `PhotoRAG`, `VideoRAG`, `DateParser`, `DeterministicAnswerExtractor`, and `LiveSetRewriteSchedule` (periodic compaction/rewrite scheduling). `SurrogateTierSelector` is now ported.
...
8. `AccessStatsManager`/`ImportanceScorer`/`SurrogateTierSelector` are standalone modules not yet wired into `MemoryOrchestrator::Recall()` context assembly path. Integration requires extending `BuildFastRAGContext` to use tier-aware surrogate selection and importance-based ordering.
