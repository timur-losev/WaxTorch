...
- [x] Optimize orchestrator `Flush()` no-op path by skipping index/store commit stages with zero pending mutations; add hybrid regression proving no-op flush does not trigger text/vector commit hooks (`FailOnCall(1)`) and preserves recall parity
...
- [x] Add replay guard for control characters in `WAXEM2` identity tags (ASCII control bytes rejected as malformed), with reopen regression forcing deterministic re-embed fallback
...
- [x] Harden persisted-embedding replay merge semantics for same-frame corruption: dimension-mismatched `WAXEM*` tail records no longer override earlier valid persisted embeddings for the same `frame_id`
...
- [x] Implement `QueryIntent` bitmask enum (`kAsksLocation|kAsksDate|kAsksOwnership|kMultiHop`) and `QueryType` enum (`kFactual|kSemantic|kTemporal|kExploratory`) with `std::hash<QueryType>` specialization split across namespace blocks to avoid MSVC C2908 on forward `unordered_map` usage
- [x] Implement `QueryAnalyzer` class with `Analyze()` → `QuerySignals` (entity detection, word count, quoted phrases, specificity score), `NormalizedTerms()` with 30-entry stop-word removal and simple suffix-based stemming (`ies→y`, `ing→∅`, `ed→∅`, `s→∅`), `EntityTerms()` with entity-cue keyword detection, `YearTerms()` extraction (1900–2099 regex), `DateLiterals()` parsing (full/abbreviated month+day, day-first numeric, ISO `YYYY-MM-DD` format), and `DetectIntent()` → `QueryIntent` bitmask from keyword trigger sets
- [x] Implement `ClassifyQuery()` rule-based free function: temporal triggers (`when/before/after/during/since/between/latest/recent`), factual triggers (`what/where/who/how many/how much/define/name/list/count`), semantic triggers (`why/how does/explain/describe/compare/analyze/discuss/impact`), with exploratory fallback for short/ambiguous queries
- [x] Implement `FusionWeights` struct (`bm25/vector/temporal` floats) and `AdaptiveFusionConfig` class with per-`QueryType` weight mapping, `Default()` preset (`factual=0.7/0.3`, `semantic=0.3/0.7`, `temporal=0.4/0.3/0.3`, `exploratory=0.5/0.5`), and `SetWeights()`/`Weights()` API
- [x] Add `query_analyzer_test` with 26 scenarios covering classification for all four types, intent bitmask, normalized terms with stemming, entity term detection, year extraction, date literal parsing (full/abbreviated/day-first/ISO), fusion weight defaults and custom config, and `QuerySignals` field validation
- [x] Implement BPE `TokenCounter` with cl100k_base vocab loading (`LoadVocab` from `.tiktoken` format with base64 decode), ASCII-safe pre-tokenization (contractions `'s/'t/'re/'ve/'m/'ll/'d`, letter/digit/punctuation/whitespace splitting compatible with cl100k_base), and greedy BPE merge using min-heap priority queue with generation counters for stale-pair invalidation
- [x] Add dual-mode `TokenCounter` operation: exact BPE encode/decode when vocab loaded (`HasVocab()=true`), byte-level estimation fallback (`~4 bytes/token`) when no vocab; public API surface: `Count/Encode/Decode/Truncate/CountBatch/HasVocab/VocabSize` with move semantics support
- [x] Implement token-aware `ChunkText()` utility function with `ChunkingConfig` (`target_tokens/overlap_tokens`) using `TokenCounter::Count` for budget enforcement and `TokenCounter::Truncate` for chunk sizing, with stall prevention guard (advance at least one byte per iteration)
- [x] Add `token_counter_test` with 23 scenarios covering estimation mode (empty/short/longer text, truncate, truncate no-op, count batch, zero/negative max_tokens), BPE mode (vocab load from `Sources/Wax/RAG/Resources/cl100k_base.tiktoken`, encode/decode round-trip, determinism, count, truncate, count batch, invalid vocab path), chunking (short text single chunk, long text multiple chunks, empty, zero target, BPE-based chunks with budget verification, zero overlap), and move semantics
- [x] Integrate `AdaptiveFusionConfig` into unified search via `UnifiedSearchAdaptive()` function: classifies query text with `ClassifyQuery()`, retrieves per-type `FusionWeights` from config, adapts `SearchRequest.mode.alpha = weights.bm25`, and routes through existing `UnifiedSearchWithCandidates`; non-hybrid mode passes through without reclassification
- [x] Add 3 adaptive fusion test scenarios: query-type-aware weight selection (temporal query → temporal alpha), non-hybrid passthrough (text-only mode unmodified), and custom `AdaptiveFusionConfig` override
- [x] Implement `ExtractiveSurrogateGenerator` with sentence segmentation (boundaries: `.!?\n;` with abbreviation guards and minimum-length merge), content-aware scoring formula (`min(wordCount,40)` base + digit bonus `6.0` + colon bonus `4.0` + bullet bonus `2.0` + backtick bonus `2.0` + uniqueness×`3.0`, with short<4 penalty `×0.25` and long>80 penalty `×0.7`)
- [x] Implement MMR (Maximal Marginal Relevance) diversity selection in `ExtractiveSurrogateGenerator` with Jaccard word-set similarity for inter-sentence redundancy detection, configurable diversity weight (`lambda=0.7`), greedy candidate selection loop, and position-preserving reordering of selected sentences to maintain source-text reading order
- [x] Implement hierarchical surrogate tier generation in `ExtractiveSurrogateGenerator::GenerateTiers()`: `full` tier = top-8 scored items joined by `\n`, `gist` tier = top-3 items joined by space, `micro` tier = single top-scored item; with `SurrogateTierConfig` struct providing `full_max_tokens/gist_max_tokens/micro_max_tokens` and three presets (`Default{100,25,8}`, `Compact{50,15,5}`, `Verbose{150,40,12}`)
- [x] Add optional `TokenCounter` integration in `ExtractiveSurrogateGenerator` constructor for accurate tier truncation when BPE vocab is available
- [x] Add `surrogate_generator_test` with 16 scenarios covering empty/short/single-sentence input, multi-sentence extraction with diversity, scored sentence ranking, hierarchical tier generation (full/gist/micro non-empty), compact/verbose tier configs, and `TokenCounter`-integrated tier truncation
- [x] Implement `MaintenanceOptions` struct (`max_frames`, `max_wall_time_ms`, `surrogate_max_tokens=60`, `overwrite_existing=false`, `enable_hierarchical=true`, `tier_config=SurrogateTierConfig::Default()`) and `MaintenanceReport` struct (`scanned_frames/eligible_frames/generated_surrogates/superseded_surrogates/skipped_up_to_date/did_timeout`)
- [x] Implement `OptimizeSurrogates()` maintenance runner: scans all committed frames via `FrameMetas()`, filters by `status==0` (active) + no `superseded_by` + non-empty payload + non-surrogate (`WAXSURR1` prefix detection), reads source content via `FrameContent()`, generates surrogates (hierarchical or single-tier based on `enable_hierarchical` flag), stores as new store frames via `Put()`, and batch-commits every 64 generated surrogates
- [x] Add hierarchical surrogate encoding format in maintenance runner: `WAXSURR1\nFULL:<full>\nGIST:<gist>\nMICRO:<micro>` lightweight text-based payload (avoids JSON dependency) with magic header for recursive-processing detection on re-runs
- [x] Add wall-time deadline enforcement in `OptimizeSurrogates()` using `steady_clock` with optional `max_wall_time_ms` (checked at top of each frame iteration), and `max_frames` limit applied to eligible (not scanned) frame count
- [x] Add overwrite-existing logic in `OptimizeSurrogates()`: scans `supersedes` chain to detect frames with existing surrogates, skips when `overwrite_existing=false`, supersedes old surrogate frame with new one when overwriting
- [x] Add forward-compatible metadata population in `OptimizeSurrogates()`: `source_frame_id`, `surrogate_algo` (`extractive_v1`), `surrogate_version` (`1`), `surrogate_max_tokens`, `surrogate_format` (`hierarchical_v1` when applicable) — stored via `Put()` metadata parameter even though current C++ binary format does not yet persist per-frame metadata
- [x] Add `maintenance_test` with 16 scenarios covering empty store, single frame, multiple frames (3 source → 3 surrogates), `max_frames` limit, wall-time deadline (`max_wall_time_ms=0`), deleted frame skipping, superseded frame skipping, empty/whitespace payload skipping, hierarchical tier generation (WAXSURR1+FULL/GIST/MICRO payload verification), non-hierarchical fallback, re-run surrogate detection (WAXSURR1 prefix prevents recursive processing), overwrite-existing behavior, report count consistency invariant, custom `SurrogateTierConfig::Compact()`, default options, and zero `max_frames`
- [x] Wire `TokenCounter` into orchestrator chunking: extract `ChunkContentWhitespace()` from old `ChunkContent()`, add `ChunkContentTokenAware(const TokenCounter*, ...)` BPE-aware path that falls back to whitespace when counter is `nullptr` or `!HasVocab()`, keep `ChunkContent()` default path as whitespace-based for backward compatibility; 0 regressions across 20/20 CTest
- [x] Implement `AccessStatsManager` with thread-safe (mutex-guarded) frame access statistics: `FrameAccessStats` struct (`frame_id/access_count/first_access_ms/last_access_ms`), saturating overflow on `uint32_t access_count`, `NowMs()` wall-clock helper, single/batch record, single/multi get, prune by active set, export/import for persistence
- [x] Add `access_stats_test` with 13 scenarios covering init, record access, saturating overflow, manager CRUD, batch access with duplicates, multi-get, missing frame, prune by active set, export/import roundtrip, import replaces, prune empty set, and wall-clock access
- [x] Implement `ImportanceScorer` with exponential decay scoring: `age_component = exp(-age_hours / half_life)`, `frequency_component = min(1.0, log(count+1) / 5.0)`, `recency_component = exp(-hours_since_access / recency_half_life)`, weighted average normalized to [0,1], configurable via `ImportanceScoringConfig` (weights, half-lives)
- [x] Add `importance_scorer_test` with 11 scenarios covering default config, brand new frame, old frame, recent access boost, high frequency, stale access, custom weights, zero weights fallback, very old frame → zero, score range validation, and future timestamp clamping
- [x] Implement `SurrogateTierSelector` with variant-based policy dispatch: `std::variant<TierPolicyDisabled, TierPolicyAgeOnly, TierPolicyImportance>` via `std::visit`, `SurrogateTier` enum (`kFull/kGist/kMicro`), `AgeThresholds` and `ImportanceThresholds` structs, query specificity boost for importance-based selection, `ExtractTier()` WAXSURR1 hierarchical format parser, preset constructors `TierPolicyAgeBalanced()`/`TierPolicyImportanceBalanced()`
- [x] Add `tier_selector_test` with 18 scenarios covering disabled policy, age-based (recent/medium/old/boundaries/custom thresholds), importance-based (new+high freq, old unaccessed, medium score, query boost, custom thresholds), ExtractTier (hierarchical, legacy passthrough, empty tier, malformed data), default constructor, and millisecond conversion
- [x] Extend `FastRAGConfig` in `types.hpp` with `FastRAGMode` enum (`kFast/kDenseCached`), `expansion_max_bytes`, `max_surrogates`, `surrogate_max_tokens`, answer-focused ranking fields (`enable_answer_focused_ranking/answer_rerank_window/answer_distractor_penalty`), query-aware tier selection (`enable_query_aware_tier_selection`), and `deterministic_now_ms` for test control
- [x] Wire `TokenCounter*` through `MemoryOrchestrator` constructor and activate `ChunkContentTokenAware()` in `Remember()` path (BPE-aware chunking when vocab loaded, whitespace fallback otherwise)
- [x] Implement `RerankingHelpers` namespace in search module: `ContainsTentativeLaunchLanguage()` (tentative/unconfirmed date language detector), `ContainsMovedToLocationPattern()` (moved/move to Capitalized pattern), `LooksDistractorLike()` (distractor content detector)
- [x] Implement `IntentAwareRerank()`: composite scoring with term recall (0.55) / precision (0.25), entity coverage (numeric 1.95 / alpha 1.25 / general 0.30), year coverage (1.25), date key coverage (1.15), intent-specific boosts (location: moved-to +1.60, date: launch +1.70, ownership: owns/owner +1.10), tentative language penalty (scaled), draft memo penalty, distractor penalty (-0.40), window-limited reranking with stable sort preservation
- [x] Wire `IntentAwareRerank()` into `UnifiedSearchWithCandidates()` pipeline (activated when query is non-empty, max_window = clamp(top_k*2, 10..32))
- [x] All 23 CTest targets pass (3 new test suites: access_stats 13, importance_scorer 11, tier_selector 18 = 42 new scenarios)
- [x] Implement `DeterministicAnswerExtractor` with 12 regex patterns (deployment ownership, generic ownership, appointment date/time, moved-to city, flight destination, allergy, preference, pet name, adoption date, launch clause), intent-aware candidate selection (ownership+date, travel, allergy, location, date, communication style, pet), relevance scoring (term recall 0.70/precision 0.30, entity 0.95, year 1.45, date key 1.25), and lexical sentence fallback
- [x] Add `answer_extractor_test` with 23 scenarios (37 checks): empty/whitespace items, bracket removal, deployment/generic ownership, ownership+date combined, launch date, generic date fallback, appointment date/time, moved-to city, flight destination, allergy, preference, pet name+adoption date, lexical sentence fallback, first-text fallback, multiple items relevance, RAGItem overload, sentence splitting, regex match/no-match, travel-location priority, ownership-only
- [x] Implement `TextChunker` with batch `Chunk()` and streaming `Stream()` modes (callback-based early stop), BPE-aware path via `TokenCounter` with whitespace fallback, `ComputeRanges()` utility matching Swift's `tokenCountChunkRanges()` algorithm
- [x] Add `text_chunker_test` with 13 scenarios (28 checks): range computation (empty, single, multiple, no-overlap, exact-fit), chunk (short/long/empty/zero-target), stream (short/long/early-stop), batch-stream parity
- [x] Implement `LiveSetRewriteSchedule` config struct (enabled, check_every_flushes, min_dead_payload_bytes, min_dead_payload_fraction, minimum_compaction_gain_bytes, minimum_idle_ms, min_interval_ms, verify_deep, destination_directory, keep_latest_candidates) and `LiveSetRewriteOptions` (overwrite_destination, drop_non_live_payloads, verify_deep) and `LiveSetRewriteReport`
- [x] Wire `AccessStatsManager` into `MemoryOrchestrator`: record frame accesses on each `Recall()` for importance-based tier selection. Wire `SurrogateTierSelector` into orchestrator constructor (importance-balanced policy when `enable_query_aware_tier_selection` is true, disabled otherwise). Expose `GetAccessStats()` for persistence
- [x] All 25 CTest targets pass (2 new test suites: answer_extractor 23, text_chunker 13 = 36 new scenarios, cumulative 65 checks)
...
| `cpp/third_party/usearch` | Added as real git submodule gitlink at pinned lockfile commit (`40d127f472e9073875566f0e9308c0302b89100a`) | Codex |
| `cpp/third_party/sqlite` | Added as real git submodule gitlink at pinned lockfile commit (`9bc017423c5f841eb55a3d9e711356c19ac86dd5`) | Codex |
| `cpp/third_party/googletest` | Added as real git submodule gitlink at pinned lockfile commit (`e9907112b47255d50b4d343e7e2160bce8dc85d1`) | Codex |
...
| `cpp/scripts/verify_submodules.py` | Added gitlink-presence policy mode (`--require-gitlinks-present` / `WAXCPP_REQUIRE_GITLINKS_PRESENT`) with explicit missing-path diagnostics for required submodules | Codex |
...
| `cpp/include/waxcpp/fts5_search_engine.hpp` | Added fail-on-call testing hooks (`SetCommitFailOnCall` / `ClearCommitFailOnCall`) for precise staged-commit failure injection | Codex |
...
| `cpp/tests/unit/wax_store_write_test.cpp` | Added deterministic corruption regression for mixed recovered+local pending replay across repeated reopen/commit cycles: inject WAL header-byte corruption between reopen and commit while validating cached-apply parity for `put/delete/supersede/putEmbedding` and final TOC/counter invariants | Codex |
...
| `cpp/include/waxcpp/query_analyzer.hpp` | Added `QueryIntent` bitmask enum, `QueryType` enum with `std::hash` specialization (split namespace blocks for MSVC C2908), `QuerySignals` struct, `FusionWeights` struct, `AdaptiveFusionConfig` class with per-type weight mapping + `Default()` preset, `QueryAnalyzer` class with `Analyze/NormalizedTerms/EntityTerms/YearTerms/DateLiterals/DetectIntent`, and `ClassifyQuery()` free function | Claude |
| `cpp/src/rag/query_analyzer.cpp` | Implemented 30-entry stop-word set, entity-cue keyword detection, suffix-based stemming (`ies→y/ing→∅/ed→∅/s→∅`), date literal parsing (full/abbreviated month+day, day-first numeric, ISO `YYYY-MM-DD`), intent detection keyword triggers, and rule-based classification with temporal/factual/semantic/exploratory trigger sets | Claude |
| `cpp/tests/unit/query_analyzer_test.cpp` | Added 26 scenarios: classification for all four query types, intent bitmask detection, normalized terms with stemming, entity term extraction, year extraction, date literal parsing (full/abbreviated/day-first/ISO), fusion weight defaults, custom fusion config, and `QuerySignals` field validation | Claude |
| `cpp/include/waxcpp/token_counter.hpp` | Added `TokenCounter` class (`Count/Encode/Decode/Truncate/CountBatch/HasVocab/VocabSize`, move semantics), `ChunkingConfig` struct (`target_tokens/overlap_tokens`), and `ChunkText()` free function | Claude |
| `cpp/src/rag/token_counter.cpp` | Implemented base64 decoder for `.tiktoken` vocab, ASCII-safe cl100k_base-compatible pre-tokenization (contractions `'s/'t/'re/'ve/'m/'ll/'d`, letter/digit/punctuation/whitespace splitting), greedy BPE merge with min-heap priority queue + generation counters for stale-pair invalidation, byte-level estimation fallback (`~4 bytes/token`), and `ChunkText` with overlap + stall prevention | Claude |
| `cpp/tests/unit/token_counter_test.cpp` | Added 23 scenarios: estimation mode (empty/short/longer/truncate/no-op/batch/zero/negative), BPE mode (vocab load from `Sources/Wax/RAG/Resources/cl100k_base.tiktoken`, encode/decode round-trip, determinism, count, truncate, batch, invalid path), chunking (short/long/empty/zero-target/BPE-budget/no-overlap), and move semantics | Claude |
| `cpp/include/waxcpp/search.hpp` | Added `#include "waxcpp/query_analyzer.hpp"` and `UnifiedSearchAdaptive()` declaration accepting `SearchRequest`, text/vector candidates, and optional `AdaptiveFusionConfig` | Claude |
| `cpp/src/rag/search.cpp` | Implemented `UnifiedSearchAdaptive()`: classifies query with `ClassifyQuery()`, retrieves per-type `FusionWeights` from config, adapts `SearchRequest.mode.alpha = weights.bm25`, routes through `UnifiedSearchWithCandidates`; non-hybrid mode passthrough | Claude |
| `cpp/tests/unit/search_test.cpp` | Added 3 adaptive fusion scenarios: query-type-aware weight selection (temporal→temporal alpha), non-hybrid passthrough, and custom `AdaptiveFusionConfig` override | Claude |
| `cpp/include/waxcpp/surrogate_generator.hpp` | Added `SurrogateTierConfig` struct with `full_max_tokens/gist_max_tokens/micro_max_tokens` and `Default()/Compact()/Verbose()` presets, `SurrogateTiers` struct (`full/gist/micro` + `version`), `ExtractiveSurrogateGenerator` class with optional `TokenCounter*` constructor, `Generate()` single-tier, and `GenerateTiers()` hierarchical | Claude |
| `cpp/src/rag/surrogate_generator.cpp` | Implemented sentence segmentation (`.!?\n;` boundaries with abbreviation guards), content-aware scoring (`min(wordCount,40)` + digit `6.0` + colon `4.0` + bullet `2.0` + backtick `2.0` + uniqueness×`3.0`, short<4 `×0.25`, long>80 `×0.7`), Jaccard word-set similarity, MMR greedy diversity selection (`lambda=0.7`), position-preserving reordering, and tier generation (full=8/`\n`, gist=3/space, micro=1) | Claude |
| `cpp/tests/unit/surrogate_generator_test.cpp` | Added 16 scenarios: empty/short/single-sentence input, multi-sentence extraction with diversity, scored ranking, hierarchical tier generation (full/gist/micro non-empty), compact/verbose configs, `TokenCounter`-integrated truncation | Claude |
| `cpp/include/waxcpp/maintenance.hpp` | Added `MaintenanceOptions` struct (`max_frames/max_wall_time_ms/surrogate_max_tokens=60/overwrite_existing=false/enable_hierarchical=true/tier_config`), `MaintenanceReport` struct (`scanned/eligible/generated/superseded/skipped/did_timeout`), and `OptimizeSurrogates()` declaration | Claude |
| `cpp/src/rag/maintenance.cpp` | Implemented `OptimizeSurrogates()`: committed frame scan via `FrameMetas()`, eligibility filter (`status==0` + no `superseded_by` + non-empty + non-`WAXSURR1`), hierarchical encoding (`WAXSURR1\nFULL:<full>\nGIST:<gist>\nMICRO:<micro>`), batch commit every 64 frames, `steady_clock` wall-time deadline, `max_frames` on eligible count, overwrite-existing via supersedes chain scan, and forward-compatible metadata population (`source_frame_id/surrogate_algo/surrogate_version/surrogate_max_tokens/surrogate_format`) | Claude |
| `cpp/tests/unit/maintenance_test.cpp` | Added 16 scenarios: empty store, single frame, multiple frames (3→6), `max_frames` limit, wall-time deadline (`max_wall_time_ms=0`), deleted frame skipping, superseded frame skipping, whitespace payload skipping, hierarchical WAXSURR1+FULL/GIST/MICRO verification, non-hierarchical fallback, re-run surrogate detection, overwrite-existing, report count invariant, custom `Compact()` config, default options, zero `max_frames` | Claude |
| `cpp/src/orchestrator/memory_orchestrator.cpp` | Added `#include "waxcpp/token_counter.hpp"`, extracted `ChunkContentWhitespace()` from old `ChunkContent()`, added `ChunkContentTokenAware(const TokenCounter*, ...)` BPE-aware path with `HasVocab()` guard and whitespace fallback, kept `ChunkContent()` default path as whitespace-based for backward compatibility (0 regressions across 20/20 CTest) | Claude |
| `cpp/CMakeLists.txt` | Added `maintenance.hpp` to `WAXCPP_PUBLIC_HEADERS`, added `src/rag/maintenance.cpp` to library sources, added `waxcpp_maintenance_test` executable/target/CTest entry with MSVC `/W4 /permissive-` and PCH reuse (cumulative: also contains query_analyzer, token_counter, surrogate_generator entries from earlier commits) | Claude |
| `cpp/include/waxcpp/access_stats.hpp` | Added `FrameAccessStats` struct (frame_id/access_count/first_access_ms/last_access_ms), `NowMs()` wall-clock helper, `AccessStatsManager` class with mutex-guarded operations (record single/batch, get single/multi, prune, export/import) | Claude |
| `cpp/src/rag/access_stats.cpp` | Implemented thread-safe access stats: saturating uint32_t overflow in `RecordAccess`, mutex lock_guard in all public methods, batch access with per-frame dedup, prune by active set intersection, export/import for serialization | Claude |
| `cpp/tests/unit/access_stats_test.cpp` | Added 13 scenarios: init, record access, saturating overflow, manager CRUD, batch access with duplicates, multi-get, missing frame, prune by active/empty set, export/import roundtrip, import replaces, wall-clock access | Claude |
| `cpp/include/waxcpp/importance_scorer.hpp` | Added `ImportanceScore` struct (score + 3 components), `ImportanceScoringConfig` (weights + half-lives), `ImportanceScorer` class with `Score()` method | Claude |
| `cpp/src/rag/importance_scorer.cpp` | Implemented exponential decay scoring: `exp(-age/half_life)` for age/recency, `log(count+1)/5.0` capped for frequency, weighted average with zero-weight fallback | Claude |
| `cpp/tests/unit/importance_scorer_test.cpp` | Added 11 scenarios: default config, brand new/old frame, recent access boost, high frequency, stale access, custom/zero weights, very old→zero, score range, future timestamp clamping | Claude |
| `cpp/include/waxcpp/tier_selector.hpp` | Added `SurrogateTier` enum, `AgeThresholds`/`ImportanceThresholds` structs, `TierSelectionPolicy` variant (`Disabled/AgeOnly/Importance`), `TierSelectionContext`, `SurrogateTierSelector` class with `SelectTier()`/`ExtractTier()`, preset constructors | Claude |
| `cpp/src/rag/tier_selector.cpp` | Implemented `std::visit`-based policy dispatch, `ImportanceScorer` integration with query specificity boost, WAXSURR1 hierarchical format parser for tier extraction, legacy passthrough for non-WAXSURR1 data | Claude |
| `cpp/tests/unit/tier_selector_test.cpp` | Added 18 scenarios: disabled policy, age-based (recent/medium/old/boundaries/custom), importance-based (new+high freq, old unaccessed, medium score, query boost, custom thresholds), ExtractTier (hierarchical/legacy/empty/malformed), default constructor, ms conversion | Claude |
| `cpp/include/waxcpp/types.hpp` | Added `FastRAGMode` enum (`kFast/kDenseCached`), extended `FastRAGConfig` with mode, expansion_max_bytes, max_surrogates, surrogate_max_tokens, answer-focused ranking fields, query-aware tier selection flag, deterministic_now_ms | Claude |
| `cpp/include/waxcpp/memory_orchestrator.hpp` | Added `#include "waxcpp/token_counter.hpp"`, added `const TokenCounter* token_counter = nullptr` constructor parameter and member for BPE-aware chunking activation | Claude |
| `cpp/src/orchestrator/memory_orchestrator.cpp` | Updated constructor to accept and store `TokenCounter*`, changed `Remember()` to call `ChunkContentTokenAware(token_counter_, ...)` instead of `ChunkContent()` for BPE-aware chunking when vocab loaded | Claude |
| `cpp/include/waxcpp/search.hpp` | Added `RerankingHelpers` namespace (ContainsTentativeLaunchLanguage/ContainsMovedToLocationPattern/LooksDistractorLike), added `IntentAwareRerank()` declaration with `QueryAnalyzer` default parameter | Claude |
| `cpp/src/rag/search.cpp` | Added ~250 lines: `RerankingHelpers` implementations (tentative language, moved-to pattern, distractor detection), `IntentAwareRerank()` with composite scoring (term recall/precision, entity/year/date coverage, intent boosts/penalties), wired into `UnifiedSearchWithCandidates()` pipeline | Claude |
| `cpp/CMakeLists.txt` | Added access_stats.hpp/importance_scorer.hpp/tier_selector.hpp to public headers, access_stats.cpp/importance_scorer.cpp/tier_selector.cpp to library sources, 3 new test targets (waxcpp_access_stats_test/waxcpp_importance_scorer_test/waxcpp_tier_selector_test) | Claude |
| `cpp/include/waxcpp/answer_extractor.hpp` | Added `AnswerExtractionItem` struct, `DeterministicAnswerExtractor` class with `ExtractAnswer()` (two overloads: `AnswerExtractionItem`/`RAGItem`), public `SplitSentences()`/`FirstRegexMatch()`, private regex-based candidate extraction (12 patterns), relevance scoring, and lexical sentence fallback | Claude |
| `cpp/src/rag/answer_extractor.cpp` | Implemented 12 pre-compiled regex patterns (deployment/generic ownership, appointment date/time, moved-to city, flight destination, allergy, preference, pet name, adoption date, launch clause), intent-aware candidate selection with priority order, relevance scoring (term recall/precision, entity/year/date coverage), `OwnershipCandidates()` with topic overlap boosting | Claude |
| `cpp/tests/unit/answer_extractor_test.cpp` | Added 23 scenarios (37 checks): empty/whitespace, bracket removal, deployment/generic ownership, ownership+date combined, launch/generic/appointment dates, moved-to city, flight destination, allergy, preference, pet name+adoption, lexical fallback, first-text fallback, multi-item relevance, RAGItem overload, sentence splitting, regex helpers, travel-location priority, ownership-only | Claude |
| `cpp/include/waxcpp/text_chunker.hpp` | Added `TextChunker` class with static `Chunk()` (batch), `Stream()` (callback-based with early stop), `ComputeRanges()` utility, `ChunkRange` struct — mirrors Swift's `TextChunker` with BPE-aware and whitespace-fallback modes | Claude |
| `cpp/src/rag/text_chunker.cpp` | Implemented `ComputeRanges()` matching Swift's `tokenCountChunkRanges()` algorithm, `Chunk()` with BPE encode/decode or whitespace tokenization, `Stream()` with early-stop callback pattern | Claude |
| `cpp/tests/unit/text_chunker_test.cpp` | Added 13 scenarios (28 checks): range computation (empty, single, multiple, no-overlap, exact-fit), chunk (short/long/empty/zero-target), stream (short/long/early-stop), batch-stream parity | Claude |
| `cpp/include/waxcpp/live_set_rewrite.hpp` | Added `LiveSetRewriteSchedule` (enabled, check_every_flushes, min_dead_payload_bytes, min_dead_payload_fraction, minimum_compaction_gain_bytes, minimum_idle_ms, min_interval_ms, verify_deep, destination_directory, keep_latest_candidates, `Disabled()` preset), `LiveSetRewriteOptions` (overwrite_destination, drop_non_live_payloads, verify_deep), `LiveSetRewriteReport` (frame counts, byte sizes, duration) | Claude |
| `cpp/include/waxcpp/memory_orchestrator.hpp` | Added `#include "waxcpp/access_stats.hpp"` and `#include "waxcpp/tier_selector.hpp"`, added `AccessStatsManager access_stats_` and `SurrogateTierSelector tier_selector_` members, added `GetAccessStats()` const/non-const accessors | Claude |
| `cpp/src/orchestrator/memory_orchestrator.cpp` | Added frame access recording in both `Recall()` overloads (collect frame IDs from search results, call `access_stats_.RecordAccesses()`), initialize `tier_selector_` with `TierPolicyImportanceBalanced()` when `enable_query_aware_tier_selection` is true | Claude |
| `cpp/CMakeLists.txt` | Added answer_extractor.hpp/live_set_rewrite.hpp/text_chunker.hpp to public headers, answer_extractor.cpp/text_chunker.cpp to library sources, 2 new test targets (waxcpp_answer_extractor_test/waxcpp_text_chunker_test) | Claude |
...
- **Embedding cache**: `EmbeddingMemoizer` ported from Swift actor to C++ mutex-guarded LRU cache with O(1) get/set via intrusive doubly-linked list, batch operations, FNV-1a 64-bit hashing matching Swift's `FNV1a64`, and `EmbeddingKey::Make()` deterministic cache key generation.
...
## Handoff Notes
M1-M9 baseline and M11 hardening are well advanced. RAG module porting is substantially progressed. New in this session: `AccessStatsManager` provides thread-safe frame access tracking with saturating counts, batch recording, pruning, and export/import. `ImportanceScorer` provides exponential-decay scoring (age/frequency/recency weighted components). `SurrogateTierSelector` provides variant-based runtime tier selection (`Disabled/AgeOnly/Importance` policies) with query specificity boost and WAXSURR1 tier extraction. `IntentAwareRerank()` provides intent-aware result reranking with composite scoring (term recall/precision, entity/year/date coverage, intent-specific boosts/penalties) matching Swift weight constants. `RerankingHelpers` provides tentative language detection, moved-to-location pattern matching, and distractor detection. `FastRAGConfig` extended with mode selection, surrogate limits, answer-focused ranking, and query-aware tier selection. `ChunkContentTokenAware()` now actively wired into `MemoryOrchestrator::Remember()` via constructor-injected `TokenCounter*`. Previously ported: `QueryAnalyzer` (query classification + intent detection + term/entity/date extraction + `AdaptiveFusionConfig`), `TokenCounter` (BPE cl100k_base + estimation), `ExtractiveSurrogateGenerator` (sentence extraction + MMR diversity + hierarchical tiers), `OptimizeSurrogates()` (batch maintenance + WAXSURR1 encoding), `UnifiedSearchAdaptive()` (per-type fusion). Additionally: `DeterministicAnswerExtractor` provides offline heuristic answer extraction with 12 regex patterns, intent-aware candidate selection, and relevance scoring. `TextChunker` provides batch and streaming token-aware chunking with BPE and whitespace fallback modes. `LiveSetRewriteSchedule/Options/Report` provide compaction configuration. `AccessStatsManager` is now wired into `Recall()` for frame access tracking, and `SurrogateTierSelector` is initialized with importance-balanced policy from config. New in this sub-session: `DeterministicAnswerExtractor` is now wired into `MemoryOrchestrator::Recall()` as optional post-processing via `enable_answer_extraction` config flag, populating `RAGContext::extracted_answer`. `EmbeddingMemoizer` ported from Swift actor to C++ mutex-guarded LRU cache with O(1) get/set, batch ops, FNV-1a 64-bit hashing, `EmbeddingKey::Make()`, and `FromConfig()` factory. 20 dedicated `IntentAwareRerank` tests added covering location/date/ownership intents, entity/year disambiguation, distractor penalty, window capping, vector source influence, empty/single item edge cases, and `RerankingHelpers` unit tests. 15 `EmbeddingMemoizer` tests added covering LRU eviction, access refresh, batch ops, capacity edges, hash collision prevention, and key generation. Cumulative test coverage: 26 (query_analyzer) + 23 (token_counter) + 3 (adaptive fusion) + 16 (surrogate_generator) + 16 (maintenance) + 13 (access_stats) + 11 (importance_scorer) + 18 (tier_selector) + 23 (answer_extractor) + 13 (text_chunker) + 20 (intent_rerank) + 15 (embedding_memoizer) = 197 new RAG scenarios, 27/27 CTest pass. M3 baseline is in place: C++ parses WAL headers, detects terminal markers for replay snapshot/header cursor fast paths, scans pending mutations with Swift-compatible decode-stop semantics, validates pending putFrame payload ranges, truncates trailing bytes on open while preserving bytes referenced by pending putFrame, stores effective WAL open-state internally, supports WAL append/capacity/padding-wrap/sentinel/checkpoint behavior via `WalRingWriter`, and wires that into `WaxStore::Put/PutBatch/Delete/Supersede/Commit`. `Commit` applies pending mutations into TOC, writes new footer/header generations, and checkpoints WAL cursor state. Pending mutation replay now uses deterministic in-memory caches for all mutation kinds (`putFrame/delete/supersede/putEmbedding`) so local staged state can commit even if pending WAL bytes become externally corrupted before commit. Crash-window behavior is covered by deterministic failpoint tests for post-TOC/pre-footer, post-footer/pre-header, and single-header-published windows. Writer-lease exclusion is now enforced in `Open/Create` with `.writer.lock` sentinel semantics and reopen-after-close test coverage. Search stack baseline now includes deterministic mode-aware unified fusion (`UnifiedSearchWithCandidates`, text/vector channel routing, hybrid RRF) with intent-aware reranking as post-fusion pass, context materialization with explicit token budgeting and item-kind policy (`BuildFastRAGContext` with expanded/snippet/surrogate policy), and orchestrator `Remember/Recall` over committed store frames with deterministic BPE-aware or whitespace chunking ingest. Text and vector recall channels now run via committed two-phase indexes (stage on ingest, commit on `flush`, rebuild on reopen), with `USearchVectorEngine` metric-aware scoring parity (`cosine|dot|l2`) and MV2V metal-segment similarity consistency checks on import. Structured-memory baseline now has orchestrator-level persistence via internal store-backed fact journal and startup replay, plus recall integration as `kStructuredMemory` candidates without internal-record leakage into text/vector channels.
...
3. ~~Expand crash/fault-injection matrix for mixed recovered+local pending mutation sets under repeated reopen/commit cycles (all WAL mutation kinds).~~ — **RESOLVED**: added deterministic multi-cycle regressions including mid-cycle WAL corruption injection and parity assertions for `put/delete/supersede/putEmbedding`.
...
8. ~~`AccessStatsManager`/`ImportanceScorer`/`SurrogateTierSelector` are standalone modules not yet wired into `MemoryOrchestrator::Recall()` context assembly path.~~ — **PARTIALLY RESOLVED**: `AccessStatsManager` now records accesses on every `Recall()`, `SurrogateTierSelector` initialized with importance-balanced policy. Full integration into `BuildFastRAGContext` (tier-aware surrogate text extraction with frame metadata lookup) still pending.
9. ~~`DeterministicAnswerExtractor` is ported and tested but not yet wired into `BuildFastRAGContext` or `MemoryOrchestrator::Recall()` output pipeline.~~ — **RESOLVED**: Answer extractor now wired into both `Recall()` overloads as optional post-processing, populating `RAGContext::extracted_answer` when `enable_answer_extraction` is true.
10. `TextChunker` is ported with batch/stream modes but currently `MemoryOrchestrator::Remember()` uses the internal `ChunkContentTokenAware()` helper. Migrating to `TextChunker::Chunk()` would unify the chunking API (one implementation path).
11. `EmbeddingMemoizer` is ported and tested but not yet wired into `MemoryOrchestrator` to replace the simple `unordered_map<uint64_t, vector<float>>` cache with clear-on-overflow strategy. Integration would provide proper LRU eviction with hit rate monitoring.
12. Full tier-aware surrogate selection integration into `BuildFastRAGContext` still pending — requires frame metadata lookup + surrogate data provider callback to select surrogate tier based on `SurrogateTierSelector` output.
