...
- [x] Add `WaxStore::TryRefreshIfPublishedCommitVisible()` probe API and direct write-path regressions (`step1` no-refresh, `step2/3/4/5` refresh, no-op when generation unchanged) to support orchestrator in-process crash-window recovery
