...
7. `IntentAwareRerank()` is wired into `UnifiedSearchWithCandidates()` but does not yet have dedicated C++ tests. The logic is covered indirectly via the existing search test suite; dedicated scenarios with intent-tagged queries and known result sets would improve confidence.
...
9. `DeterministicAnswerExtractor` is ported and tested but not yet wired into `BuildFastRAGContext` or `MemoryOrchestrator::Recall()` output pipeline. Integration point: apply answer extraction on RAGContext items as optional post-processing step.
